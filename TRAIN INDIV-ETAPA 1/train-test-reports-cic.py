...
def train_and_evaluate_model(name, model, X_train, X_test, y_train, y_test):
    pdf = PDF()
    output_folder = f"/root/resultados-ml/cic-iot-2023/{name}-60test"
    ...
    # Save PDF
    pdf_output_path = f"/root/resultados-ml/cic-iot-2023/cic-iot2023-{name}-60test-classification_report.pdf"
    pdf.output(pdf_output_path)
    print(f"PDF saved for {name}")
    return name, train_time, test_time, pdf_output_path

# Read CSV using Dask
ddf = dd.read_csv('/root/bbdd/logs-zeek/cic-iot-2023-processed-12gb.csv')
# Split data into training and testing sets
X = ddf.drop(columns=['label', 'binary-label','detailed-label'])
y = ddf['label']
X_train, X_test, y_train, y_test = train_test_split(X.compute(), y.compute(), test_size=0.6, random_state=42)
