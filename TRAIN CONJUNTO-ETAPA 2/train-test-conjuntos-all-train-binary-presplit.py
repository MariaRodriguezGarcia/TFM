...
def train_and_evaluate_model(name, model, X_train, X_test, y_train, y_test):
    pdf = PDF()
    output_folder = f"/root/resultados-ml/conjunto/{name}-60test-mix-train-binary-presplit"
    ...
    try:
        ...
        # Save PDF
        pdf_output_path = f"/root/resultados-ml/conjunto/conjunto-{name}-60test-mix-binary-presplit-classification_report.pdf"
