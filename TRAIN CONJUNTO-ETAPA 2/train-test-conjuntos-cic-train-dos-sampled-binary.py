...
y_test = concatenated_df['binary-label']

X_test = concatenated_df.drop(columns=['label','binary-label'])

y_train = sampled_cic_df['binary-label']
...
def train_and_evaluate_model(name, model, X_train, X_test, y_train, y_test):
    pdf = PDF()
    output_folder = f"/root/resultados-ml/conjunto/{name}-60test-cic-dos-sample-iotd20v2-train-binary"
    ...
    try:
        ...
        # Save PDF
        pdf_output_path = f"/root/resultados-ml/conjunto/conjunto-{name}-60test-cic-train-dos-sample-iotd20v2-binary-classification_report.pdf"
